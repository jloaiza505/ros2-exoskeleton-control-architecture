...
public:
  FaultManagerNode() : Node("fault_manager_node"), last_published_mode_("") {
    ...
    initial_mode_ = this->declare_parameter<std::string>("initial_mode", "STARTUP");
    ...
    if (!is_valid_mode(current_mode_)) {
      RCLCPP_WARN(this->get_logger(), "Invalid initial_mode '%s', defaulting to STARTUP", initial_mode_.c_str());
...
private:
  ...
  static bool is_valid_mode(const std::string & mode) {
    return mode == "STARTUP" || mode == "IDLE" || mode == "CALIBRATION" || mode == "ASSISTIVE" || mode == "FAULT";
  }
  ...
  bool can_transition(const std::string & from, const std::string & to) const {
    ...
    if (from == "IDLE") {
      return to == "CALIBRATION" || to == "FAULT";
    }
    if (from == "CALIBRATION") {
  ...
  void on_set_mode_request(
    ...
    if (!is_valid_mode(requested_mode) || requested_mode == "STARTUP") {
